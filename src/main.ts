...
const connection = pgp()("postgres://postgres:postgres@localhost:5432/ecommerce_postgres");
...
app.post("/checkout", async function (req, res) {
	...
	for (const item of req.body.items) {
		...
		const [product] = await connection.query("select * from ecommerce_postgres.product where id_product = $1", [item.idProduct]);
		if (product) {
